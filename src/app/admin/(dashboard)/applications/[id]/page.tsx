...
export default async function ApplicationDetailPage({
  ...
  const documentEntries: { label: string; path: string | null }[] = [
    { label: "Aadhaar Card", path: application.aadhaar_document_path },
    ...
    { label: "12th Class Marksheet", path: application.class12_marksheet_path },
    {
      ...
      path: application.final_semester_marksheet_path,
    },
  ];

  const documents = await Promise.all(
    documentEntries.map(async (doc) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <Link
        href="/admin"
        className="group inline-flex w-fit items-center gap-1.5 text-sm font-medium text-brand-700 transition-colors hover:text-brand-800"
      >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="flex flex-col gap-6 lg:col-span-2">
          <Reveal delay={0}>
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[var(--shadow-elevated)]">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full bg-brand-700 px-4 py-2 text-xs font-semibold text-white transition-colors hover:bg-brand-800"
                >
                  ↓ Download / Share PDF
                </a>
              </div>
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <InfoRow
                  ...
                  value={
                    <a
                      href={`tel:+91${application.mobile_number}`}
          ...
          <Reveal delay={0.08}>
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[var(--shadow-elevated)]">
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <InfoRow
                  label="Class 10th %"
                  value={`${application.class10_percentage}%`}
                />
                <InfoRow
                  label="Class 12th %"
                  value={`${application.class12_percentage}%`}
                />
                <InfoRow
                  label="Graduation %"
                  value={
                    application.graduation_percentage != null
                      ? `${application.graduation_percentage}%`
                      : "—"
                  }
                />
                <InfoRow
                  label="Future Field of Study"
                  value={application.future_field_of_study}
                />
              </div>
            </section>
          </Reveal>
          ...
          <Reveal delay={0.16}>
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[var(--shadow-elevated)]">
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <InfoRow
                  label="Amount Requested / Month"
                  value={`₹${application.amount_requested_per_month}`}
                />
                ...
                <InfoRow
                  ...
                  value={
                    <a
                      href={`tel:+91${application.father_contact}`}
                      className="font-medium text-brand-700 hover:underline"
                    >
                ...
                <InfoRow
                  ...
                  value={`₹${application.parent_annual_income}`}
                />
              </div>
            </section>
          </Reveal>
          ...
          <Reveal delay={0.32}>
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[var(--shadow-elevated)]">
              ...
              <StaggerContainer className="mt-3 flex flex-col gap-2">
                {documents.map((doc) => (
                  <StaggerItem key={doc.label}>
                    <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50/60 px-4 py-2.5 text-sm transition-colors hover:bg-brand-50/50">
